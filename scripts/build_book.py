#!/usr/bin/env python3
"""Bygg EPUB/PDF från Driftläges kanoniska Markdown-kapitel.

Scriptet normaliserar romanens kapitelrubriker från:
    # Kapitel 1 – Stopp i kön
till Pandoc-vänlig intern form:
    # 1. Stopp i kön
...
def strip_chapter_notes(text: str) -> str:
    patterns = [
        r"\n---\s*\n\s*##\s+Kort kapitelnotering\b.*\Z",
        r"\n---\s*\n\s*##\s+Kapitelnotering\b.*\Z",
        r"\n---\s*\n\s*##\s+Efter kapitel\b.*\Z",
        r"\n---\s*\n\s*Kort kapitelnotering:\s*.*\Z",
    ]
...
def normalize_chapter(source: Path, target: Path) -> tuple[int, str]:
    text = strip_chapter_notes(source.read_text(encoding="utf-8"))
    lines = text.splitlines()
    for i, line in enumerate(lines):
        if not line.strip():
            continue
        match = CHAPTER_H1_RE.match(line)
        if not match:
            raise RuntimeError(f"{source} saknar rubrik på formen '# Kapitel N – Titel'.")
        number = int(match.group(1))
        title = match.group(2).strip()
        lines[i] = f"# {number}. {title}"
        target.write_text("\n".join(lines).rstrip() + "\n", encoding="utf-8")
        return number, title
    raise RuntimeError(f"{source} är tom.")
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        for name in names:
            ...
            if 'class="chapter-number"' in data and 'class="chapter-title"' in data:
                split_headings += 1
            if any(marker in data for marker in ("Kapitelnotering", "Kort kapitelnotering", "Efter kapitel")):
                notes_found = True
        ...
        if notes_found:
            raise RuntimeError("EPUB-fel: kapitelnoteringar finns kvar i exporten.")
...
def main() -> int:
    ...
    cover_image_metadata = metadata["cover-image"]
    cover_image = (root / "publishing" / cover_image_metadata).resolve()
    ...
    with tempfile.TemporaryDirectory(prefix="driftlage-build-") as tmp:
        temp = Path(tmp)
        normalized_dir = temp / "kapitel"
        normalized_dir.mkdir()
        normalized_chapters: list[Path] = []
        for source in chapters:
            target = normalized_dir / source.name
            normalize_chapter(source, target)
            normalized_chapters.append(target)
        ...
        # Use a temporary metadata file where cover-image is root-relative for LaTeX/Pandoc.
        temp_metadata = temp / "metadata.yaml"
        meta_text = (root / "publishing/metadata.yaml").read_text(encoding="utf-8")
        meta_text = re.sub(
            r'(?m)^cover-image:\s*.*$',
            'cover-image: "omslag/omslag-driftlage.png"',
            meta_text,
        )
        temp_metadata.write_text(meta_text, encoding="utf-8")
        ...
        if "pdf" in formats:
            ...
            if engine is None:
                print("ERROR: xelatex krävs för PDF-bygget. Installera TeX Live/XeLaTeX.", file=sys.stderr)
                return 2
            ...
            for candidate in search_roots:
                if all((candidate / filename).is_file() for filename in required_font_files.values()):
                    ...
                    break
            if font_dir is None:
                for base_path in (Path("/usr/share/texmf"), Path("/usr/share/fonts")):
                    if not base_path.exists():
                        continue
                    for regular in base_path.rglob(required_font_files["regular"]):
